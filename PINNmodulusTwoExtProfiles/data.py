...
import _paths  # noqa: F401  (puts this folder, then PINNmodulusTwo, on sys.path)
from materials import load_material_properties  # from PINNmodulusTwo, unchanged
...
# Same cache locations as the base project, in the same order, plus this
# folder's own override. The bundles are NOT in git (see .gitignore), so they
# stay wherever they already are on a given machine; first existing hit wins.
_CACHE_CANDIDATES = (
    THIS_DIR / "data_cache",                    # extension-local override
    BASE_DIR / "data_cache",                    # base-project override
...
def load_ops(
    ...
    bc_scale = 1.0 / L_ref
    phys_scale = float(np.sqrt(dTdt_scale**2 + aniso_scale**2
                               + Qsrc_scale**2)) + 1e-6
    ...
    return NormBundle(
        ...
        bc_scale=bc_scale, static_feat=static_feat,
